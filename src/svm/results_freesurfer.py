"""Results using data preprocessed with freesurfer."""
...
# Load the data
df = pd.read_csv('freesurfer/Kernels/euclidean_norm.csv', index_col=0)
X = df.values
y = pd.read_csv('freesurfer/Target/age.csv', index_col=0,
                header=None).squeeze().values
...
for i in range(5):
    ...
    df_res.to_csv('freesurfer/Predictions/split-{0}.tsv'.format(i), sep='\t')

# Plot the performance on each fold
plot_results(y_trues, y_preds, marker='o', fillstyle='none',
             output_file='freesurfer/SVM.png')
